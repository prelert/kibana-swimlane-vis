...
const module = uiModules.get('prelert_swimlane_vis/prelert_swimlane_vis', [
  'kibana',
  'colorpicker.module',
]);
module
  .controller('PrelertSwimlaneVisController', function($scope, $timeout) {
    ...
    $scope.processResponse = function(response) {
      ...
      if (
        ...
        $scope.vis.aggs.bySchemaName('metric').length > 0 &&
        $scope.vis.aggs.bySchemaName('timeSplit') !== undefined &&
        $scope.vis.aggs.bySchemaName('timeSplit').length > 0
      ) {
        ...
        if (timeColumn !== undefined && metricColumn !== undefined) {
          ...
          if (
            $scope.vis.aggs.bySchemaName('viewBy') !== undefined &&
            $scope.vis.aggs.bySchemaName('viewBy').length > 0
          ) {
  ...
  .directive('prlSwimlaneVis', function(config) {
    function link(scope, element) {
      ...
      function renderSwimlane() {
        ...
        if (
          scope.vis.params.alphabetSortLaneLabels === 'asc' ||
        ...
        if (bounds) {
          ...
          earliest = moment(bounds.min)
            .startOf(aggInterval.description)
            .valueOf();
        ...
        const options = {
          ...
          legend: {
            ...
            container: angular
              .element(element)
              .closest('.prl-swimlane-vis')
              .find('.prl-swimlane-vis-legend'),
        ...
        const alternateLaneColor =
          element.closest('.theme-dark').length === 0 ? '#f5f5f5' : '#4a4a4a';
        ...
        _.each(laneIds, (labelId, i) => {
          ...
          labelText =
            labelText.toString().length < 28
              ? labelText
              : labelText.toString().substring(0, 25) + '...';
      ...
      function showTooltip(item) {
        ...
        const y = item.pageY;
        const offset = 5;
        $('<div class="prl-swimlane-vis-tooltip">' + contents + '</div>')
          .css({
            position: 'absolute',
            display: 'none',
            top: y + offset,
        ...
        $('.prl-swimlane-vis-tooltip').css(
          'left',
          x + offset + width > $win.width() ? x - offset - width : x + offset
        );
        $('.prl-swimlane-vis-tooltip').css(
          'top',
          y + height < winHeight + yOffset ? y : y - height
        );
      }
    }
